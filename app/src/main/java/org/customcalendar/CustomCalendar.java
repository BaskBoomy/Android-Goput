        ...
        switch (ev.getAction()){
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(Math.abs(valueX)>MIN_DISTANCE){
                    if(x2>x1){
//                        Toast.makeText(context, "Left to Right is swiped", Toast.LENGTH_SHORT).show();
                        slideActionLeft();
                    }
                    else{
//                        Toast.makeText(context, "Right to Left is swiped", Toast.LENGTH_SHORT).show();
                        slideActionRight();
                    }
                }
                else if(Math.abs(valueY)>MIN_DISTANCE){
//                    if(y2>y1){
//                        Toast.makeText(context, "Bottom is swiped", Toast.LENGTH_SHORT).show();
//                    }
//                    else{
//                        Toast.makeText(context, "Top is swiped", Toast.LENGTH_SHORT).show();
//
//                    }
                }
    ...
    private void setAll() {
        ...
        for(int i = 0 ; i < j ; i++) {
            ...
            if(mapDescToProp != null && mapDescToProp.get("disabled") != null && mapDescToProp.get("disabled").layoutResource != -1) {
                ...
                ((Button)btn).setBackgroundColor(Color.parseColor("#FBEFE9"));
                ((Button)btn).setTextColor(Color.parseColor("#9F807979"));
                ((Button)btn).setStateListAnimator(null);
            }
        ...
        for(int i = 0 ; i < (7-j) ; i++) {
            ...
            btnAll[index].setBackgroundColor(Color.parseColor("#FBEFE9"));
            btnAll[index].setStateListAnimator(null);
    ...
    private void readyMonthAndYear() {
        String[] arrOfMonth = getResources().getStringArray(R.array.month_of_year);
        switch (monthYearFormat) {
            case 0:
                for(int i=1;i<=12;i++){
                    if(i==selectedDate.get(Calendar.MONTH)){
                        tvMonthYear.setText(arrOfMonth[i] + " " + selectedDate.get(Calendar.YEAR));
                        break;
                    }
                }
